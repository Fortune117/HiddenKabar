		...
		if IsValid( ent ) then
			...
			if ent:IsPlayer() or ent:GetClass() == "prop_ragdoll" then
				...
				if ent:GetClass() == "prop_ragdoll" then
					if ent.Hits and ent.Hits < 4 then
						if not ent.IsGib then
							if self.Owner:Health() < 100 then
								ent.Hits = ent.Hits+1
								self.Owner:SetHealth( math.min( self.Owner:Health()+12, self.Owner:GetMaxHealth() ) )
								if ent.Hits >= 4 then
									local dmg = DamageInfo()
									dmg:SetDamage( self.Primary.Damage )
									dmg:SetInflictor( self.Weapon )
...
function SWEP:GetDamageForce()
	if self:GetPigStickMode() == 3 then
		return self.Owner:GetAimVector()*150+Vector( 0, 0, 150 ) 
	else
		return self.Owner:GetAimVector()*150
	end
end
